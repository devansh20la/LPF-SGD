...
if __name__ == "__main__":
	...
	args.cp_dir = f"checkpoints/sam_adam_{args.rho}/run_ms_{args.seed}/"
	...
	scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=1, threshold=0.01, min_lr=1e-6)
	...
	while (step < args.ts):
		...
		for batch_idx, batch in enumerate(dset_loaders['train']):
			...
			outputs = model(inputs, targets_input, inputs_mask, targets_mask, src_padding_mask, tgt_padding_mask, src_padding_mask)
			criterion(outputs.reshape(-1, outputs.shape[-1]), targets_out.reshape(-1)).backward()
