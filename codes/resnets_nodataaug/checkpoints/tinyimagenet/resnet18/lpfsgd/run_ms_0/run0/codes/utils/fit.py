    ...
    for batch_idx, inp_data in enumerate(loader, 1):
        ...
        if phase == 'train':
            with torch.set_grad_enabled(True):
                ...
                optimizer.zero_grad()
                batch_loss.backward()
                for draws in range(10):
                    noise = []
                    for mp in model.parameters():
                        noise.append(torch.zeros(mp.shape, device=mp.data.device).normal_(0, 0.01))
                        mp.data.add_(noise[-1])

                    outputs = model(inputs)
                    batch_loss = criterion(outputs, targets) / 10
                    batch_loss.backward()
                    for mp, perturb in zip(model.parameters(), noise):
                        mp.data.sub_(perturb)
                optimizer.step()
                with torch.no_grad():
                    batch_loss = criterion(outputs, targets)
