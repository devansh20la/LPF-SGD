...
def main():
	...
	args.cp_dir = f"checkpoints/esgd_{args.g0}_{args.g1}_{args.sgld_lr}/run_ms_{args.seed}/"
	...
	while (step < args.ts):
		...
		for _ in range(len(dset_loaders["train"])):
			def closure():
				nonlocal dataiter
				try:
			...
			if _ % args.print_freq == 0:
				logging.info('Batch: {0}/{1}, Train_Loss = {2}'.format(_, len(dset_loaders['train']), trainloss.avg))
