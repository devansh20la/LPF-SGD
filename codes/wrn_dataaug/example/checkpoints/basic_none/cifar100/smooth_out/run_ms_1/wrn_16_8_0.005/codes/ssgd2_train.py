...
if __name__ == "__main__":
    ...
    args.cp_dir = f"checkpoints/{args.dtype}_new_model/ssgd2/run_ms_{args.seed}/{args.depth}_{args.width_factor}_{args.M}_{args.std}"
    while True:
    ...
    for epoch in range(args.epochs):
        ...
        loss, err1, eta1_mtr, eta2_mtr = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        for batch_idx, batch in enumerate(dataset.train, 1):
            ...
            with torch.set_grad_enabled(True):
                ...
                loss_1 = criterion(predictions, targets) / args.M
            ...
            with torch.set_grad_enabled(True):
                predictions = model(inputs)
                loss_2 = criterion(predictions, targets) / args.M
            ...
            with torch.no_grad():
                f = loss_2 - loss_1
                eta_1 = torch.sign(f) / (1 + loss_1)
                eta_2 = (torch.sign(f) * (1 + loss_1) + torch.abs(f)) / ((1 + loss_1)**2)
            
            with torch.set_grad_enabled(True):
                (eta_1*loss_1 + eta_2*loss_2).backward()

            with torch.no_grad():
                loss.update(loss_1, inputs.shape[0])
                eta1_mtr.update(eta_1, inputs.shape[0])
                eta2_mtr.update(eta_2, inputs.shape[0])
        ...
        scheduler(epoch)
        writer.add_scalar('params/std', std, epoch)
        writer.add_scalar('params/eta_1', eta1_mtr.avg, epoch)
        writer.add_scalar('params/eta_2', eta2_mtr.avg, epoch)
