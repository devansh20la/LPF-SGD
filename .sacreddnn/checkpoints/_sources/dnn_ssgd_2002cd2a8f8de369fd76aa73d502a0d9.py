...
def train(loss, model, device, train_loader, optimizer, std_rate_fn, ep):
    model.train()
        ...
        for _ in range(itr):
            ...
            with torch.no_grad():
                ...
                for mp in model.parameters():
                    ...
                    temp.normal_(0, std_rate_fn(len(train_loader)*(ep-1) + batch_itr)*mp.view(-1).norm().item() + 1e-16)
                    noise.append(temp)
...
def main(_run, _config):
    ...
    for epoch in range(args.last_epoch + 1, args.epochs + 1):
        train(loss, model, device, train_loader, optimizer, std_rate_fn, epoch)
