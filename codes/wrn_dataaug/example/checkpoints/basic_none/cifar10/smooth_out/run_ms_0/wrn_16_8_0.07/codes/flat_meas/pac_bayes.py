...
def pac_bayes(model_func, mcmc_itr, eps, theta_init=False, tol=1e-6, verbose=False):
    ...
    logger.info(f"{'Itr':^10} {'sigma':^10} {'sigma_min':^10} {'sigma_max':^10} {'d':^10} {'base_loss':^10} {'curr_loss':^10}")
    for itr in range(10**7):
        ...
        if itr % 1 == 0 and verbose:
            logger.info(f"{itr:2.3E}, {sigma:2.3E}, {sigma_range[0]:2.6E}, {sigma_range[1]:2.6E}, {d:2.3E}, {base_loss:2.3E}, {curr_loss:2.3E}")
    ...
    if verbose:
        logger.info(f"Sigma found:{sigma}")
...
def _pac_bayes(func, theta_init, theta_star, mcmc_itr, data_size, eps, tol=1e-6, verbose=False):
    ...
    if verbose:
        logger.info("Computing PAC bayes bound")
    ...
    for itr in range(10**7):
        ...
        if itr % 100 == 0 and verbose:
            logger.info(f"itr:{itr:2.3E}, sigma:{sigma:2.3E},d:{d.item():2.3E}, base_loss:{base_loss.item():2.3E}, curr_loss:{func(theta).item():2.3E}")
