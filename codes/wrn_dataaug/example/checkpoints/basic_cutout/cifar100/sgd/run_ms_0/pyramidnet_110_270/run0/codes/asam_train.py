...
if __name__ == "__main__":
    ...
    parser.add_argument("--rho", default=1.0, type=float, help="Rho parameter for ASAM.")
    ...
    args.cp_dir = f"checkpoints/{args.img_aug}/{args.dtype}/" + \
                  f"asam/run_ms_{args.seed}/{args.mtype}_{args.depth}_{args.width_factor}_{args.rho}"    

    args.cp_dir = f"{args.cp_dir}"
    ...
    if args.mtype == "wrn":
        ...
        if args.width_factor == 270:
            bottleneck = False
        else:
            bottleneck = True
        model = PyramidNet(args.depth, args.width_factor, args.num_classes, bottleneck=bottleneck).to(device)
        nesterov = True
    else:
