...
from imagenet_models import load_model as load_imagenet_model
from models import load_model
from training_utils import flax_training
...
flags.DEFINE_string(
    'output_dir', 'checkpoints/', 'Directory where the checkpoints and the tensorboard '
...
def main(_):
  ...
  logging.get_absl_handler().use_absl_log_file(log_dir=output_dir)
  logging.info(FLAGS.flag_values_dict())

  for file in glob.glob("**/*.py", recursive=True):
      if "checkpoints" in file or "tf_data" in file or "results" in file:
          continue
  ...
  noised_model = jax.tree_map(lambda a: a + jax.random.normal(prng_key, shape=a.shape) * (jnp.linalg.norm(a)*1 + 1e-16),
                              model)

  norm = 0
  for e in jax.tree_leaves(jax.tree_multimap(lambda a,b: jnp.sum(jnp.square(a - b)), model, noised_model)):
    norm += e

  print(jnp.sqrt(norm))
  quit()
  #   print(jnp.sqrt(sum([jnp.sum(jnp.square(e))] ))))
  # quit()
