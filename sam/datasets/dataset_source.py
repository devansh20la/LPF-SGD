...
class CifarDatasetSource(TFDSDatasetSource):
  ...
  def __init__(self, batch_size: int, name: str, image_level_augmentations: str,
               ...
               datadir: str, image_size: Optional[int] = None):
    ...
    if FLAGS.use_test_set:
      ...
      self._train_ds = tfds.load(name, split=train_split,data_dir=datadir).cache()
      self._test_ds = tfds.load(name, split='test',data_dir=datadir).cache()
      ...
      self._train_ds = tfds.load(name, split='train[:45000]', data_dir=datadir).cache()
      self._test_ds = tfds.load(name, split='train[45000:]', data_dir=datadir).cache()
...
class FashionMnist(TFDSDatasetSource):
  ...
  def __init__(self, batch_size: int, image_level_augmentations: str,
    ...
    if FLAGS.use_test_set:
      self._train_ds = tfds.load('fashion_mnist', data_dir="/gen_v_sharp/sam/tf_data/", split='train').cache()
      self._test_ds = tfds.load('fashion_mnist', data_dir="/gen_v_sharp/sam/tf_data/", split='test').cache()
      logging.info('Used test set instead of validation set.')
      self.num_training_obs = 60000
    else:
      self._train_ds = tfds.load('fashion_mnist',data_dir="/gen_v_sharp/sam/tf_data/", split='train[:54000]').cache()
      self._test_ds = tfds.load('fashion_mnist',data_dir="/gen_v_sharp/sam/tf_data/", split='train[54000:]').cache()
