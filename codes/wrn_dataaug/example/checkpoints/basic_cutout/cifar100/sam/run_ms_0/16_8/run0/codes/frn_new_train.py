...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(dataset.train, 1):
            ...
            # g1
            grads = torch.autograd.grad(loss.mean(), model.parameters(), only_inputs=True)
            with torch.no_grad():
                for mp, gp in zip(model.parameters(), grads):
                    mp.add_(gp.mul(args.lam_2))
            ...
            predictions = model(inputs)
            loss = criterion(predictions, targets)
            grads2 = torch.autograd.grad(loss.mean(), model.parameters(), only_inputs=True)

            # going back to without noise and accumulating true update
            with torch.no_grad():
                for mp, gp1, gp2 in zip(model.parameters(), grads, grads2):
                    mp.data.sub_(gp1.mul(args.lam_2))
                    if mp.grad is not None:
                        mp.grad.add_(gp1.mul(1 - alpha).add(gp2*alpha)).div_(args.M)
                    else:
                        mp.grad = gp1.mul(1-alpha)
                        mp.grad.add_(gp2*alpha)
                        mp.grad.div_(args.M)
